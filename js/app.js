...
function renderProducts(list) {
  ...
  if (!list?.length) {
    gridEl.innerHTML = `<div class="card" style="padding:16px;">No products found.</div>`;
    return;
  }
  ...
  const html = list.map((p) => {
    ...
    return `
      <article class="card product" data-id="${p.id}" data-category="${p.category ?? "all"}">
        <div class="product__media">
          <div class="product__tag ${tagStyle}">${p.tag ?? "Producto"}</div>
        ...
        <div class="product__body">
          ...
          <div class="product__actions">
            <button class="btn btn--secondary" type="button" data-action="details" data-id="${p.id}">Detalles</button>
...
function renderCart() {
  ...
  if (!items.length) {
    ...
    // FIX: limpiar items al mostrar mensaje de carrito vacío (bug: quedaban los últimos items aunque no se veían)
...
function syncStockUI() {
  ...
  // Re-render product grid to refresh disabled state
  applyQueryAndRender();
}
...
function addToCart(productId) {
  ...
  // Cart.add already validates stock and alerts; still ok
  cart.add(p, 1);
...
function decQty(productId) {
  ...
  // If qty goes to 0, remove item (so it disappears)
  if (newQty <= 0) {
    cart.remove(productId);
  } else {
...
function checkout() {
  ...
  // “Finalizar” limpia el carrito y devuelve stock (por lógica de A)
  cart.clear();
  persistAll();
  renderCart();
  syncStockUI();
